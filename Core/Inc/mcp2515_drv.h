...
#ifndef INC_MCP2515_DRV_H_
#define INC_MCP2515_DRV_H_
#ifdef __cplusplus
extern "C" {
#endif
...
//#define true (1==1)
//#define false !(true)
#define  CAN_MSG_MAX_DATA_LENGTH 8

#define HW_DELAY HAL_Delay
#define GET_TICK_MS  HAL_GetTick
#define HW_DELAYMICROSECS delayMicroseconds
...
typedef struct CanbusCfg
{
    ...
    uint8_t mcpMode;
    uint8_t dataLen;
    uint8_t dataBuf[CAN_MSG_MAX_DATA_LENGTH];
    uint8_t nReservedTx;

    void (*ChipSelectFp)(void);
    void (*ChipUnSelectFp)(void);
    void (*SPIReadWriteFp)(uint8_t);
    void (*SPIWriteFp)(uint8_t);
    uint8_t (*SPIReadFp)(void);
...
*********************************************************************************************************/
//uint8_t sendMsgBuffer(CanbusConfig_t* canCfg,unsigned long id, uint8_t ext, uint8_t len,  uint8_t* buf);
...
*********************************************************************************************************/
uint8_t sendMsgBufandWait(CanbusConfig_t* canCfg,uint8_t status, unsigned long id, uint8_t ext, uint8_t rtrBit, uint8_t len, const uint8_t* buf, int8_t wait_sent);
